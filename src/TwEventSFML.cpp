...
#include "MiniSFML24.h" // a subset of SFML 1.6 headers needed to compile TwEventSFML.cpp
    ...
    */
    ...
    case sf::Event::KeyPressed:
        ...
        case sf::Keyboard::Key::Space:
            ...
            break;
        default:
            if (event->key.code >= sf::Keyboard::Key::F1 && event->key.code <= sf::Keyboard::Key::F15)
                key = TW_KEY_F1 + event->key.code - sf::Keyboard::Key::F1;
            else if (s_KMod & TW_KMOD_ALT) 
            {
                if (event->key.code >= sf::Keyboard::Key::A && event->key.code <= sf::Keyboard::Key::Z)
                {
                    if (s_KMod & TW_KMOD_SHIFT)
                        key = 'A' + event->key.code - sf::Keyboard::Key::A;
                    else
                        key = 'a' + event->key.code - sf::Keyboard::Key::A;
                }
            }
